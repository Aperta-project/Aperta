...
class AuthenticatedPage(StyledPage):
    ...
    def get_db_submission_data(short_doi):
        """
        Provided a manuscript ID, queries the database for current publishing_state,
    ...
    def post_new_discussion(self, topic='', msg='', mention='', participants=''):
        ...
        if participants:
            for participant in participants:
                ...
                # This sleep needs to be at least 1s as we are so slow in forming the matching
                #   selection list
                time.sleep(1)
    ...
    def post_discussion(self, msg='', mention=''):
        ...
        try:
            ...
            raise (ElementDoesNotExistAssertionError,
                   'This may be caused due to dynamic buttons '
    ...
    def validate_withdraw_banner(self, journal):
        """
        A method to validate the withdraw banner. Only valid in the context of either the
          paper_viewer or the workflow page.
    ...
    def validate_recent_activity_entry(self, msg, full_name=''):
        ...
        for ra_entry in ra_entries:
            try:
                ...
                if full_name:
                    try:
                        assert full_name in self._get(
                            self._recent_activity_table_user_full_name).text
