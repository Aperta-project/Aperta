...
class CoverLetterTask(BaseTask):
    ...
    def __init__(self, driver):
        ...
        self._task_body_base_locator = 'div.task-main-content.custom-card-task '
        self._instructions_text_first_p = (
            By.CSS_SELECTOR, self._task_body_base_locator + 'div.card-content-view-text')
        self._instructions_text_last_p = (By.CSS_SELECTOR, 'div.card-form-text')
    ...
    def validate_cover_letter_task_styles(self):
        ...
        expected_instructions_questions = ['What is the scientific question you are addressing?',
                                           'What is the key finding that answers this question?',
                                           'What is the nature of the evidence you provide in support of your '
                                           'conclusion?',
                                           'What are the three most recently published articles that are relevant '
                                           'to this question?',
                                           'What significance do your results have for the field?',
                                           'What significance do your results have for the broader community (of '
                                           'biologists and/or the public)?',
        ...
        assert tinymce_editor_instance_id and tinymce_editor_instance_iframe, 'Cover letter text area is not ' \
                                                                              'present in the task!'
    ...
    def fill_and_complete_letter_textarea(self):
        ...
        logging.info('Editor instance is: {0}'.format(tinymce_editor_instance_id))
        self._scroll_into_view(tinymce_editor_instance_iframe)
    ...
    def upload_letter(self, letter='random'):
        ...
        self._wait_for_text_be_present_in_element(self._uploaded_attachment_item_link, formatted_file_name)
        return letter
    ...
    def replace_letter(self, letter='random', exclude=''):
        ...
        self._get_uploaded_item_element()
        if letter == 'random':
            letter = random.choice(cover_letters)
        fn = os.path.join(os.getcwd(), letter)
    ...
    def download_letter(self):
        ...
        if self.get_last_uploaded_letter_file():
            ...
            pattern = "\([^\d]*(\d+)[^\d]*\)"
            original_file_name = str(re.sub(pattern, '', original_file_name))
            ...
            original_file_path = os.path.join(original_working_dir, original_file_path)
