...
class InviteReviewersCardTest(CommonTest):
    ...
    def test_smoke_invite_reviewers_styles_elements(self):
        ...
        self.create_article(title='Test Invite Reviewers - elements and styles',
    ...
    def test_core_invite_reviewers_actions(self):
        ...
        reviewers from new document creation through inviting reviewer, validation of the invite
        ...
        dashboard_page.click_create_new_submission_button()
        mmt = 'OnlyInitialDecisionCard'
        self.create_article(title='Test Invite Reviewers - actions',
                            journal='PLOS Wombat', type_=mmt, random_bit=True)
        manuscript_page = ManuscriptViewerPage(self.getDriver())
        manuscript_page.page_ready_post_create()
        short_doi = manuscript_page.get_paper_short_doi_from_url()
        ...
        dashboard_page.validate_invitation_in_overlay(mmt=mmt,
                                                      ...
                                                      paper_id=paper_id)
        invite_response, response_data = dashboard_page.accept_or_reject_invitation(
                manuscript_title,
                role='Reviewer')
        ...
        try:
            test_for_role = PgSQL().query('SELECT role_id FROM assignments '
                                          'WHERE user_id = %s '
    ...
    def test_core_invite_rescind_reinvite(self):
        ...
        dashboard_page.click_create_new_submission_button()
        mmt = 'OnlyInitialDecisionCard'
        self.create_article(title='Test Invite Reviewers - Invite Rescind Reinvite',
                            journal='PLOS Wombat', type_=mmt, random_bit=True)
        manuscript_page = ManuscriptViewerPage(self.getDriver())
        manuscript_page.page_ready_post_create()
        short_doi = manuscript_page.get_paper_short_doi_from_url()
        ...
        try:
            ...
            test_for_role = False
        if invite_response == 'Accept' and test_for_role:
            assert test_for_role == reviewer_role_for_env, \
                'assigned role, {0}, is not the expected ' \
                'value: {1}'.format(test_for_role, reviewer_role_for_env)
        elif invite_response == 'Decline':
            assert not test_for_role
            # search for reply
            reasons, suggestions = PgSQL().query('SELECT decline_reason, reviewer_suggestions '
    ...
    def test_core_invite_email_template_edit(self):
        ...
        # The following line is useful for debugging issues where a previous test fails, leaving us
        #   in an unexpected directory. We have repeatedly had failures around this, so for
        #   the time, I would like to leave this in place and commented.
        # current_path = os.getcwd()
        ...
        dashboard_page.click_create_new_submission_button()
        mmt = 'OnlyInitialDecisionCard'
        self.create_article(title='Test Invite Reviewers - email templates',
                            journal='PLOS Wombat', type_=mmt, random_bit=True)
        manuscript_page = ManuscriptViewerPage(self.getDriver())
        manuscript_page.page_ready_post_create()
        short_doi = manuscript_page.get_paper_short_doi_from_url()
    ...
    def test_invited_reviewer_report_state(self):
        ...
        dashboard_page.click_create_new_submission_button()
        mmt = 'OnlyInitialDecisionCard'
        self.create_article(title='Test Invite Reviewers - report state',
                            journal='PLOS Wombat', type_=mmt, random_bit=True)
        manuscript_page = ManuscriptViewerPage(self.getDriver())
        manuscript_page.page_ready_post_create()
        short_doi = manuscript_page.get_paper_short_doi_from_url()
