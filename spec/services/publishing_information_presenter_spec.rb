...
describe PublishingInformationPresenter do
  ...
  it "#html returns complete publishing information" do
    %i(title journal_name generated_at downloader_name).each do |method|
  ...
  it "#downloader_name returns the name of the user the PDF was generated for in a p tag at the end" do
    expect(publishing_information_presenter.downloader_name).to eq("<p id='downloader-name'><em>Generated for #{downloader.full_name}</em></p>")
  end
end
