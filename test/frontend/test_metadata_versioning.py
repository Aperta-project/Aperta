...
from Base.Resources import staff_admin_login, internal_editor_login, ascii_only_users, \
    creator_login21, creator_login22
...
class MetadataVersioningTest(CommonTest):
    ...
    def test_metadata_versioning(self):
        ...
        new_prq = {'q1': 'Yes', 'q1_child_answer': 'Results and data',
                   ...
                   'q4': 'New Collection',
                   'q5': 'More Short Title'}
        # excluding non_orcid_users_logins from test as test is failing on Author card completion
        ...
        dashboard_page._wait_on_lambda(
            lambda: len(dashboard_page._gets(dashboard_page._dashboard_invite_title)) >= 1)
        ...
        paper_viewer._wait_on_lambda(lambda: paper_viewer.is_task_open('Additional Information'))
        ...
        # assert Diff Icons
        diff_icons = paper_viewer._gets(paper_viewer._paper_sidebar_diff_icons)
        assert diff_icons
        # the following rigamarole is necessitated by the too significant differences between the
        #     paper_viewer page in diff mode versus its normal presentation.
        paper_viewer = ManuscriptViewerPage(self.getDriver())
        paper_viewer.click_task('Additional Information', diff_mode=True)
        # Version differences
