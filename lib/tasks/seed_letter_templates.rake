      ...
      Journal.all.each do |journal|
        ...
        LetterTemplate.where(name: 'Review Reminder - Before Due', journal: journal).first_or_initialize.tap do |lt|
          ...
          lt.body = <<-TEXT.strip_heredoc
            ...
            <p>Thank you again for agreeing to review “{{ paper.title }}” for {{ journal.name }}. This is a brief reminder that we hope to receive your review comments on the manuscript by {{ review.due_at | date "%B %-d, %I%P %Z" }}. Please let us know as soon as possible, by return email, if your review will be delayed.</p>
        ...
        LetterTemplate.where(name: 'Review Reminder - First Late', journal: journal).first_or_initialize.tap do |lt|
          ...
          lt.body = <<-TEXT.strip_heredoc
            <p>Dear Dr. {{ reviewer.last_name }}</p>
            <p>This is a reminder that your review of the PLOS Biology manuscript “{{ paper.title }}” was due to be received by  {{ review.due_at | date "%B %-d, %I%P %Z" }}.</p>
        ...
        LetterTemplate.where(name: 'Review Reminder - Second Late', journal: journal).first_or_initialize.tap do |lt|
          ...
          lt.body = <<-TEXT.strip_heredoc
            <p>Dear Dr. {{ reviewer.last_name }}</p>
            <p>This is a reminder that your review of the PLOS Biology manuscript “{{ paper.title }}” was expected by the agreed due date, {{ review.due_at | date "%B %-d, %I%P %Z" }}. At this stage we urgently need the review in order to proceed with the editorial process.</p>
