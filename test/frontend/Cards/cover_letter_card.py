...
class CoverLetterCard(BaseCard):
    ...
    def __init__(self, driver):
        ...
        self._instructions_text_first_p = (By.CSS_SELECTOR, 'div.task-main-content div.card-content-view-text')
        ...
        self._cover_letter_textarea_noneditable = (By.XPATH, '//div[contains(@data-editor, "cover_letter--text")]/p')
    ...
    def validate_styles(self):
        ...
        expected_instructions_questions = ['What is the scientific question you are addressing?',
                                           ...
                                           'conclusion?',
                                           'What are the three most recently published articles that are relevant to '
                                           'this question?',
        ...
        for i, question in enumerate(instructions_questions):
            assert question.text == \
                expected_instructions_questions[i], 'The instructions question {0}: {1} is not the expected: ' \
    ...
    def validate_uploaded_file_download(self, uploaded_file):
        ...
        uploaded_file_md5 = \
            hashlib.md5(open(os.path.join(original_working_dir + '/frontend/assets/coverletters/',
                                          urllib.parse.unquote_plus(uploaded_file)), 'rb').read()).hexdigest()
        downloaded_file_md5 = hashlib.md5(open(os.path.join('/tmp', newest_file), 'rb').read()).hexdigest()
