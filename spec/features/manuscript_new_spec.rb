...
feature 'Create a new Manuscript', js: true, sidekiq: :inline! do
  ...
  let!(:journal) { FactoryGirl.create :journal, :with_roles_and_permissions, :with_test_cards, pdf_allowed: true }
  let!(:non_pdf_journal) { FactoryGirl.create :journal, :with_roles_and_permissions, :with_test_cards, pdf_allowed: false }
  let!(:papers) { [] }
