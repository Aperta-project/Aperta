...
describe StandardTasks::PaperReviewerTask do
  ...
  describe "#paper_roles" do
    context "when no roles exist" do
      specify { expect(StandardTasks::PaperReviewerTask.new(phase: phase).paper_roles).to be_empty }
    end
    ...
    context "when roles exist" do
      ...
      it "returns the roles" do
        ...
        expect(StandardTasks::PaperReviewerTask.new(phase: phase).paper_roles).to match_array roles
      end
    end
  end
  ...
  describe "#reviewer_ids" do
    let(:paper) { Paper.create! short_title: 'Role Tester', journal: Journal.create! }
    let(:task) { StandardTasks::PaperReviewerTask.create! phase: paper.task_manager.phases.first }
