...
class PaperTrackerPage(AuthenticatedPage):
    ...
    def __init__(self, driver, url_suffix='/'):
        ...
        self._paper_tracker_table_paper_id_th = (
            By.CSS_SELECTOR, 'th.paper-tracker-paper-id-column')
        self._paper_tracker_table_preprint_id_th = (
            By.CSS_SELECTOR, 'th.paper-tracker-paper-preprint-id-column')  # preprint DOI
        self._paper_tracker_table_version_date_th = (
            By.CSS_SELECTOR, 'th.paper-tracker-date-column')
        self._paper_tracker_table_submit_date_th = (
            By.CSS_SELECTOR, 'th.paper-tracker-date-column+th.paper-tracker-date-column')
        ...
        self._paper_tracker_table_he_th = (
            By.CSS_SELECTOR, 'th.paper-tracker-handling-editor-column')
    ...
    def _get_paper_list(journal_ids, sort_by='short_doi', reverse=False):
        ...
        for journal in journal_ids:
            journal_papers += PgSQL().query(
                    'SELECT short_doi, title, doi, submitted_at, '
        ...
        # next the papers with no submitted_at populated (I think this is limited to withdrawn
        # papers with NULL s_a date) APERTA-3023 - this ordering is non-deterministic at present so
        # this case will fail until this defect is resolved and the test case updated as needed.
        withdrawn_papers = []
        for journal in journal_ids:
        ...
        try:
            if sort_by == 'publishing_state':
                ...
                papers = sorted(papers, key=lambda x: x[sort_by_d[sort_by]].lower(),
                                reverse=reverse)
        except AttributeError:
            # For sorting by date
            papers = sorted(papers, key=lambda x: x[sort_by_d[sort_by]], reverse=reverse)
        logging.debug(
            'Here is the list of papers from db sorted by {0}:\n{1}'.format(sort_by, papers))
        return papers

    def validate_search_execution(self):
        """
        Validates the search and saved search elements of the page, executes a search, saves that
        query as a saved search, clears the current search, executes the saved search, then deletes
        that saved search.
        ...
        assert 'Title keyword or Manuscript ID number' in \
               search_input.get_attribute('placeholder'), search_input.get_attribute('placeholder')
        ...
        # get needed links to click on the saved search
        saved_search_link, delete_saved_search = self._get_saved_search_links()
        saved_search_link.click()
        # we need to get the links one more time, as the elements might be stale after
        # the document has been refreshed
        saved_search_link, delete_saved_search = self._get_saved_search_links()
        self._wait_for_element(delete_saved_search)
    ...
    def _get_saved_search_links(self):
        """
        The function to find links to saved search with 'Saved Search from Automation Test' text
        :return: saved_search_link - link to the saved search, delete_saved_search - link to
        the delete icon
        """
        saved_search_link = ''
        delete_saved_search = ''
        ...
        for search in saved_search_div:
            ...
            try:
                ...
                continue
            self._scroll_into_view(search_link)
            saved_search_link = search
            delete_saved_search = search.find_element(*self._paper_tracker_saved_search_delete_link)
            break
        return saved_search_link, delete_saved_search
    ...
    def validate_pagination(self, username):
        ...
        assert '{0} found'.format(total_count) in initial_paginination.text, \
            'Total count: {0} is not equal to page displayed total {1}'.format(
                    total_count, initial_paginination.text)
    ...
    def validate_table_presentation_and_function(self, total_count, journal_ids, user_type):
        ...
        # First step is to grab the papers from the db in the correct order for comparison to the
        # page. This is a bit complicated because the ordering should be by submitted_at, but,
        # some of the return set has a NULL value for submitted at. We put these first in an as yet
        # unknown ordering. I am going to build two lists then join them.
        # First the papers with submitted_at populated
        submitted_papers = []
        if total_count > 0:
            for journal in journal_ids:
                journal_papers = PgSQL().query(
                    'SELECT short_doi, title, doi, submitted_at, paper_type, '
        ...
        if total_count > 0:
            ...
            for count, row in enumerate(table_rows):
                ...
                self._paper_tracker_table_tbody_title = (
                    By.XPATH,
                    '//tbody/tr[{0}]/td[@class="paper-tracker-title-column"]/div/a'.format(
                        count + 1))
                self._paper_tracker_table_tbody_manid = (
                    By.XPATH,
                    '//tbody/tr[{0}]/td[@class="paper-tracker-paper-id-column"]/div/a'.format(
                        count + 1))
                ...
                self._paper_tracker_table_tbody_paptype = (
                    By.XPATH,
                    '//tbody/tr[{0}]/td[@class="paper-tracker-type-column"]'.format(count + 1))
                self._paper_tracker_table_tbody_status = (
                    By.XPATH,
                    '//tbody/tr[{0}]/td[@class="paper-tracker-status-column"]'.format(count + 1))
                self._paper_tracker_table_tbody_members = (
                    By.XPATH,
                    '//tbody/tr[{0}]/td[@class="paper-tracker-members-column"]'.format(count + 1))
                self._paper_tracker_table_tbody_he = (
                    By.XPATH,
                    '//tbody/tr[{0}]/td[@class="paper-tracker-handling-editor-column"]'.format(
                        count + 1))
                self._paper_tracker_table_tbody_ce = (
                    By.XPATH,
                    '//tbody/tr[{0}]/td[@class="paper-tracker-cover-editor-column"]'.format(
                        count + 1))
                title = self._get(self._paper_tracker_table_tbody_title)
                if not db_papers[count][3]:
                    logging.warning(
                        'Paper without Date Submitted: id {0}'.format(db_papers[count][0]))
                ...
                if papers[count][0]:
                    ...
                    assert db_title.split() == page_title.split(), \
                        'DB: {0}\nPage: {1}\n Page Row: {2}'.format(
                                db_title.split(),
                                page_title.split(),
                                count + 1)
                ...
                # only on publication do we use the full manuscript id, until then we strip the
                #   front part: '10.1371/journal.' from the doi
                ...
                logging.debug('Page Article Type: {0}\n'
                              'DB Article Type: {1}\n'
                              'Paper Row Count: {2}'.format(paptype.text, db_papers[count][4],
                                                            count + 1))
            ...
            paptype_th = self._get(
                self._paper_tracker_table_paper_type_th).find_element_by_tag_name('a')
            paptype_th.click()
            time.sleep(1)
            self._paper_tracker_table_tbody_paptype = (
            ...
            assert article_type == papers[0][5], \
                'Article Type in page: {0} != Article Type in DB: {1}'.format(article_type,
                                                                              papers[0])
            ...
            if we_have_null_date_submitted:
                logging.warning(
                    'Skipping validation of Submission/Version Date ordering as we have Papers '
                    'without Date Submitted')
            else:
                logging.info('Sorting by Submission Date ASC')
                date_th = self._get(
                    self._paper_tracker_table_submit_date_th).find_element_by_tag_name('a')
                date_th.click()
                time.sleep(2)
                # check order
                ...
                date_th = self._get(
                    self._paper_tracker_table_version_date_th).find_element_by_tag_name('a')
                date_th.click()
                time.sleep(2)
                # check order
            ...
            logging.info(
                'Found {0} on page, searching for result in db.'.format(paper_tracker_title))
            ...
            # The title validation ends up being a rather difficult animal because the ordering
            #  presented in the GUI is actually a complex sort in the descending case of title desc
            #  and short_doi ascending.
    ...
    def _validate_current_sort(self, journal_ids, sort_by='', reverse=False):
        """
        Perform a validation of a sort of the paper tracker including documents in journal_ids,
          sorting by sort_by, passing in whether the sort is DESC (True) or ASC (False)
        :param journal_ids: journals for which the papers should be found
