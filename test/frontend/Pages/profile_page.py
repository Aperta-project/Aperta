...
class ProfilePage(AuthenticatedPage):
    ...
    def __init__(self, driver):
        ...
        self._add_affiliation_title_field = (By.CSS_SELECTOR,
                                             'div.department + div.department > input')
        ...
        self._add_affiliation_end_date_field = (By.CSS_SELECTOR,
                                                'div.form-group > div > input + input')
        ...
        self._add_affiliation_email_error = (By.CSS_SELECTOR,
                                             'div.required.email div.error-message')
    ...
    def page_ready(self):
        ...
        A method to validate that the profile page has fully loaded. At time of this writing, it
            appears as if the orcid information is drawing last. I suspect this will change at
            some point.
    ...
    def validate_initial_page_elements_styles(self, username):
        ...
        if unlinked:
            ...
            assert oid_link.get_attribute('href') == oid_href, \
                'Orcid link on page: {0} != expected link: ' \
                '{1}'.format(oid_link.get_attribute('href'), oid_href)

            # The display of this delete icon is no longer available to the end user. Also, due to
            #     a bug staff will only see it on their own profile - not that of other users
            #     (APERTA-8564).
        ...
        assert 'Any changes to your username or email will be updated ' \
               'in Aperta on your next login.' in profile_link_subtext.text, \
            profile_link_subtext.text
        ...
        try:
            ...
            self._get(self._profile_affiliation_aff_text)
            # APERTA-8178 Typo
        ...
        if not existing_affiliation:
            ...
            assert add_form_subtext.text == 'Enter most recent affiliations first', \
                add_form_subtext.text
            ...
            for item in country_list:
                assert item in country_list_page_text, 'Item: {0} in NED Country list that is ' \
                                                       'not found in the page!'.format(item)
            # Then check that there are no extra items in the Country list from page.
            for item in country_list_page_text:
                assert item in country_list, 'Extra item in country list from page: {0} - is NED ' \
            ...
            assert 'time at institution:' in affiliation_dates_label.text, \
                affiliation_dates_label.text
            ...
            self._get(self._profile_affiliation_dates)
            # APERTA-8381
            # # There is a problem that this data is mal-structured in the application so you get
            #     way more than you ask for here:
    ...
    def validate_add_affiliation_validations(self, user):
        ...
        add_done_btn.click()
        try:
            self._get(self._add_affiliation_email_error)
        except ElementDoesNotExistAssertionError:
    ...
    def transient_affiliation_exists(self):
        ...
        :return: True and the affiliation_list for the found affiliation, False and an empty list
            if no affiliation found.
        """
        aff_list = []
        self.set_timeout(3)
        try:
    ...
    def edit_affiliation(self, affiliation_to_edit_list):
        ...
        Accepts a list defining the elements of an affiliation (institution, department, title,
            country, start date, end date, and associated email), finds that list, then edits it,
            then returns the newly edited list
        ...
        """
        logging.debug('Edit affiliation called for affiliation: '
                      '{0}'.format(affiliation_to_edit_list))
        ...
        for aff in all_affiliations:
            ...
            title = aff.find_element(*self._profile_affiliation_title)
            country = aff.find_element(*self._profile_affiliation_country)
            aff.find_element(*self._profile_affiliation_dates)
            # APERTA-8381
            # start_date, end_date = date_range.text.split(' - ')
            aff_email = aff.find_element(*self._profile_affiliation_email)
            if institution.text == expected_inst and department.text == expected_dept \
                ...
                # WARNING: Due to some absolutely grisly bugs in the date fields when editing an
                #     affiliation this very specific order must be followed to avoid having the
                #     Start field spontaneously clear.
                # Also, beware that the input is really non-functional when doing date format
                #     input. If the date and month are not impossible to confuse (use a date
                #     greater than 12), the interpreter produces cracktastic results.
                # WARNING APERTA-8382
                end_date_field = self._get(self._add_affiliation_end_date_field)
                # end_date_field.click()
    ...
    def delete_affiliation(self, affiliation_to_delete_list):
        """
         ...
         :param affiliation_to_delete_list: a list defining the elements of an affiliation. Dates
             are expected in mm/dd/yyyy form.
         :return: void function
         """
        expected_inst, expected_dept, expected_title, expected_country, expected_sdate, \
        ...
        for aff in all_affiliations:
            ...
            # start_date, end_date = date_range.text.split(' - ')
            aff_email = aff.find_element(*self._profile_affiliation_email)
            if institution.text == expected_inst and department.text == expected_dept and \
                title.text == expected_title and country.text == expected_country and \
                    aff_email.text == expected_email:
    ...
    def clear_transients(self):
        """
        A housekeeping function to ensure profiles don't fill up with "transient" affiliation
            entries
        :return: void function
        """
        self.set_timeout(1)
    ...
    def validate_affiliation(self, affiliation_definition_list):
        """
        Being passed an affiliation definition list, validates the presence of an affiliation
            listing in view mode on the user profile with the values passed.
        ...
        for aff in all_affiliations:
            ...
            if institution.text == expected_inst and department.text == expected_dept and \
                ...
                return
            logging.info(u'{0}, {1}, {2}, {3}, {4}'.format(institution.text,
                                                           department.text,
                                                           title.text,
                                                           country.text,
                                                           aff_email.text))
    ...
    def validate_no_affiliation(self, affiliation_definition_list):
        """
        Being passed an affiliation definition list, validates the affiliation listing is not
            present in view mode on the user profile with the values passed.
        :param affiliation_definition_list: a seven item list representing the definition of an
          affiliation
        :return: void function
        """
        ...
        for aff in all_affiliations:
            ...
            if institution.text == expected_inst and department.text == expected_dept and \
                ...
                raise(ValueError, 'Affiliation: {0} failed '
                                  'deletion'.format(affiliation_definition_list))
