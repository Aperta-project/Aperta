  ...
  namespace :migrate do
    ...
    task aperta_11299_data_migration_competing_interest_card_html_tags: :environment do
      card_contents = CardContent.joins(:content_attributes)
                          ...
                          .where('content_attributes.string_value LIKE ?', 'Please provide details about any and all competing interests in the box below. Your response should begin with this statement:%')
      ...
      card_contents = CardContent.joins(:content_attributes)
                          .where('content_attributes.name': 'text')
                          .where('content_attributes.string_value LIKE ?', '<ol class="question-list"><li class="question"><div class="question-text"><p>You are responsible for recognizing and disclosing on behalf of all authors any competing interest that could be perceived to bias%')
      if card_contents.empty?
        raise Exception, 'No matching cards were found - has Competing Interests been migrated to a custom card yet?'
      end
      CardContent.transaction do
