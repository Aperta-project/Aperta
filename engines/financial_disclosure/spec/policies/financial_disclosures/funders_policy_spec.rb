...
describe FinancialDisclosure::FundersPolicy do
  let(:policy) { FinancialDisclosure::FundersPolicy.new(current_user: user, funder: funder) }
  let(:paper) { FactoryGirl.create(:paper, :with_tasks) }
  let(:task) { paper.phases.first.tasks.first }
  let(:funder) { FinancialDisclosure::Funder.new(task: task) }
