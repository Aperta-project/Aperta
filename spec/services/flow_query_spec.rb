  ...
  describe "#tasks" do
    ...
    context "scoping tasks by journal" do
      ...
      it "is not applicable to site admins" do
        flow = FactoryGirl.build(:flow, state_query: 'completed')
      ...
      it "scopes the tasks to the flow's journal if the flow is not a default flow" do
        flow = FactoryGirl.build(:flow, state_query: 'completed', journal: user_journal)
        tasks = FlowQuery.new(user, flow).tasks
      ...
      it "scopes tasks to the user's journals if flow is a default flow" do
        flow = FactoryGirl.build(:flow, :default, title: 'My tasks', state_query: 'completed')
        tasks = FlowQuery.new(user, flow).tasks
    ...
    context "scoping tasks by assigned" do
      ...
      it "scopes tasks to assigned to the user if assigned query is true" do
        flow = FactoryGirl.build(:flow, :default, title: 'My tasks', assigned_query: 'true')
        tasks = FlowQuery.new(user, flow).tasks
      ...
      it "scopes tasks to unassigned if the assigned query is false" do
        flow = FactoryGirl.build(:flow, :default, title: 'My tasks', assigned_query: 'false')
        tasks = FlowQuery.new(user, flow).tasks
    ...
    context "scoping tasks by state" do
      ...
      it "does not extend scope if the state_query query is nil" do
        flow = FactoryGirl.build(:flow, :default, title: 'My tasks', assigned_query: 'true')
        tasks = FlowQuery.new(user, flow).tasks
      ...
      it "scopes tasks to incomplete task if the state query is incomplete" do
        flow = FactoryGirl.build(:flow, :default, title: 'My tasks', state_query: 'incomplete')
        tasks = FlowQuery.new(user, flow).tasks
    ...
    context "scoping tasks by role" do
      ...
      it "it scopes tasks by role if role is given" do
        flow = FactoryGirl.build(:flow, :default, title: 'My tasks', role_query: "Admin")
        tasks = FlowQuery.new(user, flow).tasks
    ...
    context "scoping tasks by type" do
      ...
      it "scopes tasks by type" do
        flow = FactoryGirl.build(:flow, :default, type_query: "StandardTasks::PaperAdminTask")
        tasks = FlowQuery.new(site_admin, flow).tasks
    ...
    context "default scopes" do
      ...
      context "Up for grabs" do
        it "returns tasks that are up for grabs" do
          flow = FactoryGirl.build(:flow, :default, assigned_query: 'false', state_query: 'incomplete')
          tasks = FlowQuery.new(user, flow).tasks
      ...
      context "My tasks" do
        it "returns tasks that are 'my tasks'" do
          flow = FactoryGirl.build(:flow, :default, state_query: 'incomplete', assigned_query: 'true')
          tasks = FlowQuery.new(user, flow).tasks
      ...
      context "Done" do
        it "returns tasks that are done" do
          flow = FactoryGirl.build(:flow, :default, state_query: 'completed', assigned_query: 'true')
          tasks = FlowQuery.new(user, flow).tasks
