...
describe StandardTasks::PaperAdminTaskPresenter do
  ...
  describe "#data_attributes" do
    ...
    let(:task) do
      ...
      paper = Paper.create! title: "Foo bar",
        ...
        user: author
      task = StandardTasks::PaperAdminTask.create! completed: true,
    ...
    subject(:data_attributes) { StandardTasks::PaperAdminTaskPresenter.new(task).data_attributes }

    it_behaves_like "all tasks, which have common attributes" do
      let(:card_name) { 'standards-paper-admin' }
      let(:assignees) { [] }
      let(:assignee_id) { nil }
    end
