  ...
  class RegisterDecisionScenario < PaperScenario
    def reviews
      @reviews ||= [].tap do |reviews|
        ...
        reports_with_num, reports_without_num = reports.partition(&:reviewer_number)
        reports = reports_with_num.sort_by(&:reviewer_number) + reports_without_num.sort_by(&:submitted_at)
