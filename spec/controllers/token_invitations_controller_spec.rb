...
describe TokenInvitationsController do
  ...
  describe 'POST #decline' do
    ...
    context 'when the token points to an "invited" invitation' do
      ...
      it 'rejects the invitation' do
        do_request
        expect(invitation.reload.state).to eq("rejected")
      end
    ...
    context 'when the token points to a "rejected" invitation' do
      let(:invitation) { FactoryGirl.create(:invitation, :rejected) }
  ...
  describe 'POST #feedback' do
    ...
    context "the invite is in a state other than 'rejected'" do
      let(:invitation) { FactoryGirl.create(:invitation, :accepted) }
      it 'redirects to the root page' do
        do_request
        expect(response).to redirect_to(root_path)
    ...
    context "the invite is in a 'rejected' state" do
      let(:invitation) { FactoryGirl.create(:invitation, :rejected) }
      ...
      context 'the invite already has either feedback or suggestions' do
        let(:invitation) { FactoryGirl.create(:invitation, :rejected, decline_reason: "reasons") }
        it 'does not update the invite' do
          do_request
          invitation.reload
          expect(invitation.reviewer_suggestions).to be_nil
