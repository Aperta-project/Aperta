...
from Base.Resources import users, academic_editor_login, editorial_users
...
class ReviewerCandidatesTaskTest(CommonTest):
    ...
    def test_smoke_reviewer_candidates_styles(self):
        ...
        rcc.validate_styles(reviewer_user, paper_id)

    def test_core_add_reviewer_candidate(self):
        """
        test_reviewer_candidates_task: Validates the submission of a recommended or opposed
            reviewer.
            Validates the presentation of the data submitted from the task on the workflow card.
                Note that this also validates the styling of the candidate display on the card -
                as this isn't covered in the style validation due to lack of an entry.
        :return: void function
        """
        ...
        rev_cand_task = ReviewerCandidatesTask(self.getDriver())
        time.sleep(2)
        choice, reason = rev_cand_task.complete_reviewer_cand_form(reviewer_user)
        ...
        rcc = ReviewerCandidatesCard(self.getDriver())
        rcc.validate_styles(reviewer_user, paper_id)
        rcc.check_initial_population(reviewer_user, choice, reason)
    ...
    def test_core_reviewer_candidates_permissions(self):
        """
        test_reviewer_candidates_task: Validates the access permissions for the reviewer candidates
            task
        ...
        rev_cand_task = ReviewerCandidatesTask(self.getDriver())
        # Note that one cannot use the normal task ready method here because we suppress the
        #     completion button when the editing permission is disabled. So the dread sleep...
        time.sleep(2)
