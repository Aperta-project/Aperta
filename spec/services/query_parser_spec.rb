  ...
  describe '#parse' do
    describe 'paper metadata queries' do
      ...
      it 'parses raw doi queries' do
        ...
        expect(parse.to_sql).to eq(<<-SQL.strip)
          ("papers"."doi" ILIKE '%1241251%' OR "papers"."preprint_doi_article_number" ILIKE '%1241251%')
        SQL
      end
      ...
      it 'parses framed aarx_doi queries' do
        ...
        expect(parse.to_sql).to eq(<<-SQL.strip)
          "papers"."preprint_doi_article_number" ILIKE '%1241251%'
        SQL
      end
    ...
    describe 'people queries' do
      ...
      it 'parses TASK x IS ASSIGNED TO a' do
        ...
        expect(parse.to_sql).to eq(<<-SQL.strip)
          \"tasks_0\".\"title\" ILIKE 'anytask' AND 1=0
        SQL
      end
    end
