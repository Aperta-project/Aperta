...
class AuthorsTask(BaseTask):
    ...
    def __init__(self, driver):
        ...
        self._individual_author_edit_label = (
            By. CSS_SELECTOR, 'div.individual-author-form > div > fieldset > legend')
        ...
        self._department_lbl = (By.CSS_SELECTOR,
                                'div.flex-group + div.flex-group div + div div label')
        ...
        self._designed_chkbx = (By.CSS_SELECTOR,
        ...
        self._edit_author = (By.CSS_SELECTOR, 'div.author-name')
        self._corresponding = (
            By.CSS_SELECTOR, 'input[name=author--published_as_corresponding_author]')
        ...
        self._group_inits_lbl = (
            By.CSS_SELECTOR, 'div.group-author-form > div > fieldset > div > div + div > '
                             'div > label')
        ...
        self._gauthor_contrib_lbl = (By.CSS_SELECTOR,
                                     'fieldset.author-contributions legend.required')
    ...
    def validate_author_task_styles(self):
        ...
        assert 'http://www.icmje.org/recommendations/browse/' in \
        ...
        assert authors_note.text == 'Note:  Please ensure a unique and active email address is ' \
                                    'provided for each contributing author (whether individual' \
                                    ' or group authors), and that authors are listed in the ' \
    ...
    def validate_individual_author_form_styles(self, add_new_author_btn):
        ...
        corresponding_lbl, deceased_lbl, conceptualization_lbl, investigation_lbl, \
            visualization_lbl, methodology_lbl, resources_lbl, supervision_lbl, software_lbl, \
            data_curation_lbl, project_admin_lbl, validation_lbl, writing_od_lbl, \
            writing_re_lbl, funding_lbl, formal_analysis_lbl = \
            ind_auth_form.find_elements_by_class_name('question-checkbox')
        ...
        assert not self._orcid_connect_exist(), 'ORCID Connect exists when not expected.'
        # Validate the Govt Employee section
        gquest = self._get(self._govt_employee_question)
        assert 'Is this author an employee of the United States Government?' in gquest.text, \
            gquest.text
        ghelp = self._get(self._govt_employee_help)
        assert 'Papers authored by one or more U.S. government employees are not copyrighted, ' \
               'but are licensed under a CC0 Public Domain Dedication, which allows unlimited ' \
        ...
        assert ghelp_link.get_attribute('href') == \
        ...
        assert "To submit your manuscript, please acknowledge each statement below" in \
            ack_text.text, ack_text.text
        assert agree2name_lbl.text == 'Any persons named in the Acknowledgements section of the ' \
                                      ...
                                      agree2name_lbl.text
        assert auth_criteria_lbl.text == 'All authors have read, and confirm, that they meet, ' \
                                         'ICMJE criteria for authorship.', auth_criteria_lbl.text
    ...
    def validate_group_author_form_styles(self, add_new_author_btn):
        ...
        assert author_contribs_heading_link.text == 'Contributions', \
            author_contribs_heading_link.text
        ...
        assert 'Papers authored by U.S. government organizations are not copyrighted, but are ' \
               'licensed under a CC0 Public Domain Dedication, which allows unlimited ' \
               'distribution and reuse of the article for any lawful purpose. This is a legal ' \
               'requirement for U.S. government employees.' in ghelp.text, ghelp.text
        ghelp_link = ghelp.find_element_by_tag_name('a')
        assert ghelp_link.get_attribute('href') == \
    ...
    def add_individual_author_task_action(self):
        ...
        # APERTA-10056
        institution_input.send_keys('College of Saint Benedict and Saint John\'s '
                                    'University' + Keys.ENTER)
        self.set_timeout(3)
        error_msg = self.check_for_flash_error()
        if not error_msg:
            pass
        else:
            raise AssertionError('Error message fired for institution with quote in name: '
                                 '{0}'.format(error_msg))

        affiliations = self._gets(self._affiliations)
        # select entered name from 'did-you-mean' list
        self.select_institution(affiliations[0], 'College of Saint Benedict and Saint '
                                                 'John\'s University')
    ...
    def edit_author(self, author_data):
        ...
        :param author_data: data sourced from Resources.py used to fill out author card. NOTA
            BENE: You must pass author data for an author that contains an ORCID id - otherwise
            we will fail on validation errors
        return None
        """
        ...
        assert self._orcid_connect_exist(), 'ORCID Connect does not exists when expected.'
        self.pause_to_save()
    ...
    def _orcid_connect_exist(self):
        """
        Return the existence of the ORCID connect button in individual author section
        return Boolean
        """
        orcid_connect_exist = bool()
        try:
    ...
    def validate_coauthors_elements_absence(self):
        ...
        assert self._check_for_absence_of_element(self._coauthor_confirm_lbl), \
            'Confirm label is present'
        assert self._check_for_absence_of_element(self._coauthor_decline_lbl), \
    ...
    def check_contribution(self, individual_author):
        ...
        author_contribution_chck = self._get(contribution_locator)
