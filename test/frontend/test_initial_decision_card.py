...
class InitialDecisionCardTest(CommonTest):
  ...
  def test_validate_initial_submit(self):
    ...
    paper_url = url.split('/')[0]+'//'+url.split('/')[2]+'/papers/'+paper_id
    paper_workflow_url = paper_url + '/workflow'
    ...
    # AC 2
    assert initial_decision_card._get(initial_decision_card._decision_letter_textarea).text == ''
    # AC 1 and 3
    initial_decision_card.execute_decision('invite')
    # look for alert info
    alert_msg = initial_decision_card._get(initial_decision_card._alert_info)
    ...
    # THIS IS NOT WORKING, BUT LINE 133 WORKS
    self._driver.get(paper_url)
    self._driver.get('http://www.yahoo.com')
    self._driver.navigated = True

    #dashboard_page = DashboardPage(self.getDriver())
    #dashboard_page.navigate_to(paper_url)
    time.sleep(60)
