...
test('Finds no differences when there are none', function(assert) {
  ...
  authors.map((author, idx) => {
    assert.deepEqual(authors[idx].viewing, viewing[idx], `author ${idx + 1} viewing matches`);
    assert.deepEqual(authors[idx].comparing, comparing[idx]), `author ${idx + 1} comparing matches`;
  });
...
test('Finds position differences in lists with the same authors', function(assert) {
  ...
  assert.equal(authors.length, 3, 'there are three authors');
  assert.deepEqual(authors[0].viewing, authors[0].comparing, 'first author unchanged');
  assert.deepEqual(authors[1].viewing, viewing[1]);
  assert.deepEqual(authors[2].viewing, viewing[2]);
  assert.deepEqual(authors[1].comparing, comparing[2]);
  assert.deepEqual(authors[2].comparing, comparing[1]);
});
...
test('An author was removed', function(assert) {
  ...
  assert.equal(authors[1].viewing, null);
  assert.deepEqual(authors[1].comparing, comparing[1]);
});
...
test('An author was added', function(assert) {
  ...
  assert.deepEqual(authors[1].viewing, viewing[1]);
  assert.equal(authors[1].comparing, null);
});
...
test('An author was added and removed', function(assert) {
  ...
  assert.deepEqual(authors[1].viewing, viewing[1]);
  assert.equal(authors[1].comparing, null);
  assert.equal(authors[2].viewing, null);
  assert.deepEqual(authors[2].comparing, comparing[1]);
});
