  ...
  namespace :migrate do
    namespace :nested_questions do
      desc <<-DESC.strip_heredoc
        Adds NAV customer ids as additional information for the
        Institutional Account Program nested question.

        This is used as metadata when it is exported for Billing
      DESC
      program_names =
      [
      ...
      task add_nav_ids_to_institutions_questions: :environment do
        institution = NestedQuestion.where(ident: 'plos_billing--ringgold_institution').first
        institution.nested_question_answers.each do |answer|
          institute = program_names.find { |institute| institute[:text] == answer.value }
          if institute
            nav_number = institute[:nav_customer_number]
            puts "Updating billing task #{answer.owner_id}'s ringgold answer of #{answer.value} with #{nav_number}"
            answer.update_column(:additional_data, nav_customer_number: nav_number)
          else
            puts "Unable to find matching institution for billing task #{answer.owner_id} and #{answer.value}"
          end
        end
      end
    end
