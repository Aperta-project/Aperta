    ...
    namespace :reviewers do
      ...
      task remove_participant_roles: :environment do
        ...
        User.joins(:roles).where(roles: {name: 'Reviewer Report Owner'}).map do |reviewer|
          reviewer.participations.where(assigned_to_id: set_of_reviewer_report_ids).each do
            assignment_count += 1
          end
        end
        number_of_reviewer_reports = set_of_reviewer_report_ids.count
        if assignment_count > number_of_reviewer_reports
           STDERR.puts("Number of participants is higher than the amount of Reviewer Reports. Please review the data and fix before trying again.")
           fail "Reviewer participants higher than Reviewer Reports"
        end
