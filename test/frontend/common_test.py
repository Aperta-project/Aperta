...
class CommonTest(FrontEndTest):
    ...
    def create_article(self, title='', journal='', type_='', document='', random_bit=False,
        ...
        :param document: Name of the document to upload. If blank will default to 'random', this
        will choose one of available papers
        :param format_: type of doc to use to create the initial submission. Valid values: 'any',
          'pdf', 'word'
        If preprint overlay in create sequence:
          :return title: Return the title of the article
        Else:
          :return title, poics_selection: Return the title of the article and the preprint opt-in
          selection
        """
        ...
        if document:
            ...
            if format_ == 'any':
                # polled attachments of type SourceFileAttachment between 6/1/2017 and 11/14/2017
                # to come up with this approximate breakdown
        ...
        if poics:
            logging.info(
                    'Preprint posting overlay in create sequence - need to fill out card and click '
                    'Continue...')
            ...
            poics_selection = preprint_overlay \
                .select_preprint_overlay_in_create_sequence_and_continue()
    ...
    def set_editors_in_db(paper_id):
        ...
        wombat_journal_id = \
            PgSQL().query('SELECT id FROM journals WHERE name = \'PLOS Wombat\';')[0][0]
        handling_editor_role_for_env = \
            PgSQL().query('SELECT id FROM roles WHERE journal_id = %s AND '
        ...
        cover_editor_role_for_env = PgSQL().query('SELECT id FROM roles WHERE journal_id = %s AND '
                                                  'name = \'Cover Editor\';',
                                                  (wombat_journal_id,))[0][0]
        academic_editor_role_for_env = \
            PgSQL().query('SELECT id FROM roles WHERE journal_id = %s AND '
                          'name = \'Academic Editor\';',
                          (wombat_journal_id,))[0][0]

        handedit_user_id = PgSQL().query(
                'SELECT id FROM users WHERE username = \'ahandedit\';')[0][0]
        covedit_user_id = PgSQL().query(
                'SELECT id FROM users WHERE username = \'acoveredit\';')[0][0]
        acadedit_user_id = PgSQL().query(
                'SELECT id FROM users WHERE username = \'aacadedit\';')[0][0]
    ...
    def set_staff_in_db():
        """
        Set up a internal editor, staff admin, billing_staff, publishing services and production
          staff for a given journal.
        ...
        staffadm_user_id = \
            PgSQL().query('SELECT id FROM users WHERE username = \'astaffadmin\';')[0][0]
        billstaff_user_id = \
            PgSQL().query('SELECT id FROM users WHERE username = \'abillstaff\';')[0][0]
        pubsvcs_user_id = PgSQL().query('SELECT id FROM users WHERE username = \'apubsvcs\';')[0][0]
        prodstaff_user_id = \
            PgSQL().query('SELECT id FROM users WHERE username = \'aprodstaff\';')[0][0]
    ...
    def set_freelance_eds_in_db():
        """
        Set up a freelance editor role for cover editor and handling editor in plos wombat test
        journal. This supports seeding data in a new environment
        Returns None
        """
        # Set up a handling editor, academic editor and cover editor for this paper
        wombat_journal_id = \
            PgSQL().query('SELECT id FROM journals WHERE name = \'PLOS Wombat\';')[0][0]
        freelance_editor_role_for_env = \
            PgSQL().query('SELECT id FROM roles WHERE journal_id = %s AND '
                          'name = \'Freelance Editor\';',
                          (wombat_journal_id,))[0][0]

        handedit_user_id = PgSQL().query('SELECT id FROM users WHERE username = \'ahandedit\';')[0][
            0]
        covedit_user_id = PgSQL().query('SELECT id FROM users WHERE username = \'acoveredit\';')[0][
            0]
    ...
    def set_site_admin_in_db():
        ...
        siteadmin_user_id = \
            PgSQL().query('SELECT id FROM users WHERE username = \'asuperadm\';')[0][0]
    ...
    def toggle_feature_setting_in_db(owner_id, owner_type, feature_name, enable):
        ...
        :param owner_id: The id of the owner for which the setting needs to be toggled (e.g.
          a Journal)
    ...
    def preprint_overlay_in_create_sequence(journal: str, mmt: str) -> bool:
        """
        A method that will determine for mmt if the pre-print posting overlay should be shown as
        part of the create new manuscript sequence. Tests for Preprint feature flag enablement for
        system, preprint checkbox selection for mmt, and finally presence of Preprint Posting card
        in mmt. If all three are found, return True, else False
        ...
        if not pp_ff:
            return False
        mmt_id, pp_eligible_mmt = \
            PgSQL().query('SELECT manuscript_manager_templates.id, is_preprint_eligible '
