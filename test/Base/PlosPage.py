...
class PlosPage(object):
    ...
    def __build_environment_url(self, url_suffix):
        ...
        **Returns** A string representing the whole URL from where our test starts
        ...
        baseurl = self.PROD_URL if env == 'prod' else base_url + url_suffix
        return baseurl

    def _get(self, locator):
        try:
            return self._wait.until(
                expected_conditions.visibility_of_element_located(locator)).wrapped_element
    ...
    def _gets(self, locator):
        try:
            return [x.wrapped_element for x in
                    self._wait.until(expected_conditions.presence_of_all_elements_located(locator))]
        except TimeoutException:
            logging.info(
    ...
    def _wait_on_lambda(self, wait_lambda, max_wait=30):
        ...
        if None is saved_exception:
            ...
            raise TimeoutException(
                '{0} not satisfied before {1} seconds passed'.format(lambda_src, max_wait))
        else:
            raise saved_exception

    def _iget(self, locator):
        """
        Unlike the regular _get() function, this one will be successful for elements with a width
        and or height of zero; stupid name, but suggesting 'i' for invisible as a zero
        width/height element.
        :param locator: locator
        """
        try:
            return self._wait.until(
                expected_conditions.presence_of_element_located(locator)).wrapped_element
        except TimeoutException:
            logging.error(
                '\t[WebDriver Error] WebDriver timed out while trying to identify element '
                'by {0}.'.format(locator))
    ...
    def _check_for_invisible_element(self, locator):
        ...
        try:
            return self._wait.until(
                expected_conditions.invisibility_of_element_located(locator)).wrapped_element
        except TimeoutException:
            logging.error(
                '\t[WebDriver Error] WebDriver timed out while trying to look for hidden element '
                'by {0}.'.format(locator))
    ...
    def _wait_for_text_be_present_in_element(self, locator, text,
        ...
        :param multiplier: the multiplier of Config.wait_timeout to wait for a locator
        to be not present
        """
        timeout = wait_timeout * multiplier
        self.set_timeout(timeout)
