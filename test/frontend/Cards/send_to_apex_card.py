...
class SendToApexCard(BaseCard):
        ...
        try:
            for key, value in author.items():
                if key == 'contributions':
                    assert asset_author[key] in value, \
                        'Data point from json file: {0} does not match to data point of the ' \
                        'source manuscript taken from the GUI: {1}'.format(value,
                                                                           asset_author[key])
                else:
                    assert value == asset_author[key], \
                        'Data point from json file: {0} does not match to data point of the ' \
            ...
            assert journal_title == 'PLOS Wombat', \
                'Data point from json file: {0} does not match to data point of the source ' \
                'manuscript taken from the GUI: {1}'.format(journal_title, 'PLOS Wombat')
            assert manuscript_id == short_doi, \
                'Data point from json file: {0} does not match to data point of the source ' \
                'manuscript taken from the GUI: {1}'.format(manuscript_id, short_doi)
            assert paper_abstract == manuscript_abstract, \
                'Data point from json file: {0} does not match to data point of the source ' \
                'manuscript taken from the GUI: {1}'.format(paper_abstract, manuscript_abstract)
            assert paper_title == manuscript_title, \
                'Data point from json file: {0} does not match to data point of the source ' \
                'manuscript taken from the GUI: {1}'.format(paper_title, manuscript_title)
            assert paper_type == 'generateCompleteApexData', \
                'Data point from json file: {0} does not match to data point of the source ' \
                'manuscript taken from the GUI: {1}'.format(paper_type, 'generateCompleteApexData')
            assert '/journal.{0}'.format(short_doi) in doi, \
                'Data point from json file: {0} does not match to data point of the source ' \
                'manuscript taken from the GUI: {1}'.format('/journal.{0}'.format(short_doi), doi)
    ...
    def validate_source_file_in_zip(filename, directory_path, hash_file, short_doi, file_ext):
        ...
        try:
            assert hash_file == \
                hash_file_extracted, 'The extracted document is not the same that were uploaded ' \
                                     'as source file: Uploaded hash {0} and extracted hash ' \
                                     '{1}'.format(hash_file, hash_file_extracted)
        except AssertionError:
            shutil.rmtree(directory_path)
