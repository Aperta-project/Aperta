# rubocop:disable all
# rake data:migrate:reference_json:add_institutional_accounts
namespace :data do
  namespace :migrate do
    namespace :reference_json do
      desc <<-DESC.strip_heredoc
        Adds the institutional account json to the database.

        This is referenced from Ember on the Billing Task.
      DESC
      items =
      [
      ...
      task add_institutional_accounts: :environment do
        account_list = ReferenceJson.find_or_create_by(name: "Institutional Account List")
        puts "Updating institutional accounts.."
