...
describe ExportService do
  ...
  let(:service) do
    ExportService.new export_delivery: export_delivery
  end
  ...
  describe "#make_delivery!" do
    ...
    context "the destination is preprint" do
      ...
      it "the POST request returns an error if a param value is missing" do
        ...
        expect { service.send(:make_delivery!) }.to raise_error(ExportService::ExportServiceError, /Missing required.*files/)
  ...
  describe "#package_filename" do
    it "fails if there is no manuscript ID" do
      ...
      expect { service.send(:package_filename) }.to \
        raise_error(ExportService::ExportServiceError)
    end
