...
describe StandardTasks::DeclarationTaskPresenter do
  ...
  describe "#data_attributes" do
    let(:task) do
      ...
      task = StandardTasks::DeclarationTask.create! title: "Paper Admin",
        completed: true,
        role: 'admin',
    ...
    subject(:data_attributes) { StandardTasks::DeclarationTaskPresenter.new(task).data_attributes }
    ...
    it_behaves_like "all tasks, which have common attributes" do
      ...
      let(:card_name) { 'standards-declaration' }
      let(:assignees) { [] }
    end
