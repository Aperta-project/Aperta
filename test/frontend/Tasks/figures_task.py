...
class FiguresTask(BaseTask):
    ...
    def validate_styles(self):
        ...
        assert intro_text_p2.text == (
            'We recommend that you use the PACE tool to prepare your figures for submission '
            'according to our figure requirements.'), intro_text_p2.text
        ...
        assert self._get(self._question_label).text == 'Yes - I confirm our figures comply with ' \
                                                       'the guidelines.'
        ...
        assert labels_intro.text == (
            'For .doc or .docx manuscript uploads only, figure labels (e.g. \'Fig 1\') are '
            'generated from file names and will automatically place figures above matching '
            'legends. PDF manuscripts are presented as uploaded.'), labels_intro.text
        ...
        for fig_listing in fig_listings:
            ...
            if figure_label.text == 'Unlabeled':
                error_msg_b = self._get(self._figure_listing)\
                    .find_element(*self._figure_error_msg_b)
                assert \
                    "Sorry, we didn't find a figure label in this filename. Please edit to add " \
                    "a label." in error_msg_b.text, error_msg_b.text
            else:
                try:
                    error_msg = self._get(self._figure_listing)\
                        .find_element(*self._figure_error_msg_a)
                except ElementDoesNotExistAssertionError:
    ...
    def upload_figure(self, figure2send='random', iterations=1):
        ...
        :param figure2send: Name of the figure to upload. If blank will default to 'random', this
            will choose one of available figures
    ...
    def replace_figure(self, figure2replace='', replacement_figure=''):
        ...
        :param replacement_figure: The name of the new figure, if not specified a random figure
            will be chosen.
        ...
        for figure_listing in figure_listings:
            ...
            if page_fig_name.text == figure2replace:
                ...
                # Apparently one doesn't need to click the replace button - sending the file
                #     triggers the upload.
    ...
    def delete_figure(self, figure=''):
        ...
        for figure_listing in figure_listings:
            ...
            if page_fig_name.text == figure:
                ...
                delete_btn.click()
                self._wait_on_lambda(lambda: not self.task_has_figures())
                return
        ...
        for page_fig_item in page_fig_list:
            for fig in figure:
                ...
                if fig in page_fig_item.text:
                    ...
                    time.sleep(1)
                    orig_dir = os.getcwd()
                    # If anything fails between the next line and line 333, we end up in a bogus
                    ...
                    while newest_file.split('.')[-1] == 'part':
                        ...
                        try:
                            ...
                            os.chdir(orig_dir)
                            logging.warning('Another process may deleted files from /tmp. While '
                                            'rare, this should not be considered a failure.')
                            return
                        logging.debug(newest_file.split('.')[-1])
                    newest_file = newest_file.split('/')[-1]
                    # doing the following ahead of the assert to ensure we end up in the right
                    #     directory whether the assert fails or succeeds
                    os.remove(newest_file)
                    os.chdir(orig_dir)
                    try:
                        assert fig == newest_file, 'Figure from page: {0} doesn\'t match the ' \
                                                   'newest file in the /tmp directory: {1}. This ' \
                                                   'should usually succeed but when the OS is ' \
                                                   'active it can fail and be ' \
                                                   'fine.'.format(fig, newest_file)
                    except AssertionError:
                        logging.warning('Newest file: {0} is not the file we expected: {1}. '
                                        'Another process may have written to '
                                        '/tmp'.format(newest_file, fig))
                    return
        if not matched:
    ...
    def edit_figure(self, figure=''):
        ...
        while len(figure_blocks) < 2:
            ...
            count += 1
            if count >= 9:
                raise(AssertionError, 'On edit, a minute passed without figure blocks ordering '
                                      'properly')
        for figure_block in figure_blocks:
            page_fig_name = figure_block.find_element(*self._figure_dl_link)
            ...
            while not page_fig_name.text:
                ...
                count += 1
                if count > 10:
                    raise(Exception, 'Figure block not populating correctly - not getting a '
                                     'figure name')
        ...
        if not self.task_has_figures():
            ...
            for page_fig_item in page_fig_list:
                if not page_fig_item.text:
                    ...
                    page_fig_name_list.append(page_fig_item.text)
                    for figure in fig_list:
                        # We shouldn't have to url-encode this, due to APERTA-6946 we must for now.
                        assert urllib.parse.quote_plus(figure) not in page_fig_name_list, \
                            '{0} found in {1}'.format(urllib.parse.quote_plus(figure),
                                                      page_fig_name_list)
    ...
    def task_has_figures(self):
        ...
        """
        self._reset_position_to_conformance_question()
        # if we don't have figures, there is only one div under
        #     div.liquid-container: div.liquid-child
