...
class InviteCard(BaseCard):
    ...
    def __init__(self, driver):
        ...
        self._confirm_overlay = (By.CLASS_NAME, 'confirm-overlay')
        self._button_rescind = (By.CLASS_NAME, 'button')
        # the following locators assume they will be searched for by find element within the scope
        #   of the above, enclosing div
    ...
    def validate_invite(self, invitee, mmt, title, creator, short_doi):
        ...
        for invite in invitations:
            ...
            if 'Rescinded' in invite_state.text:
                ...
                # The following 3 lines need to differentiate the specific invite
                ...
                if abstract is not None:
                    ...
                    # Eff BeautifulSoup - actually, we need to preserve some tags
                    # and some entitites so rolling my own here - this will bite me in the keyster
                    # at some point I am certain
        ...
        # see note about sleep below, same applies here with additional complication of
        # possible exceptions
        # this code block essentially says that ElementDoesNotExistAssertionError
        # is the same as len(self._gets(self._replace_attachment)) < 1
        ...
        # Used to have a long sleep here that would still occasionally fail resulting
        # in a index error below
        ...
        for item in self._gets(self._invitee_listing):
            if invitee['email'] in item.text and 'Rescinded' not in item.text:
                # So the issue is that the thing you need to click on is the thing
                # with the link/js function upon it. So in this case, it can't be the full name,
                # it must be the listing itself.
                item.click()
    ...
    def validate_response(self, invitee, response, reason='N/A', suggestions='N/A'):
        ...
        for invite in invitations:
            ...
            if 'Rescinded' in invite_state.text:
                ...
                if response == 'Accept':
                    # Review due vs. Review pending is a feature flag.
                    # Eventually we can remove Review pending
                    ...
                    assert reason in reason_suggestions, u'{0} not in {1}'\
                        .format(reason, reason_suggestions)
                    assert suggestions in reason_suggestions, u'{0} not in {1}'\
                        .format(suggestions, reason_suggestions)
    ...
    def revoke_invitee(self, invitee, role):
        ...
        for invitation in invited:
            ...
            if invitee['name'] in pagefullname.text:
                ...
                revoke.click()
                self._wait_for_element(self._get(self._confirm_overlay))
                self._get(self._button_rescind).click()
    ...
    def _validate_invitation_revocation(self, invitee, role):
        ...
        :return: void function
        """
        # It is very important to CLOSE any open invites or state information will not be present
        # where we are looking for it.
        oes = False
        self.set_timeout(5)
        try:
            ...
            if invitee['name'] in pagefullname.text:
                ...
                if 'Rescinded' not in state:
                    raise ElementExistsAssertionError(
                        'Invitation for {0} and {1} - should have been '
    ...
    def add_invitee_to_queue(self, invitee):
        ...
        self._get(self._recipient_field).send_keys(invitee['email'] + Keys.ENTER)
        self._get(self._compose_invitation_button).click()
        # In order to wait long enough for another invitee to be added, should this method
        # be called multiple times, find the "ADD TO QUEUE" button element:
        self._driver.find_element_by_class_name('button--disabled')

    def validate_email_template_edits(self):
        """
        Validates ability to expand invitation items within the queue and edit the email template.
        Edits to email templates should persist, to enable saving the template
        and sending at a later date.
        :return: void function
        """
        invitation_items = self._get(self._invitation_items).find_elements_by_class_name(
            'invitation-item')
        ...
        if len(invitation_items) > 1:
            ...
            assert 'invitation-item--closed' in first_invitation_item.get_attribute('class'), \
                first_invitation_item.get_attribute('class')
            # Only one item at most should be expanded. Select a different invitation item,
            #   and check that only it is expanded
            first_invitation_item.click()
            ...
            invitation_items = self._get(self._invitation_items).find_elements_by_class_name(
                'invitation-item')
            expanded_items = filter(
                lambda item: 'invitation-item--show' in item.get_attribute('class'),
                invitation_items)
            expanded_items = list(expanded_items)
            assert len(expanded_items) == 1, 'There is more than one expanded item: {0}'.format(
                expanded_items)
        ...
        self.pause_to_save()  # time.sleep(1)
        self._get(self._invitation_save_button).click()
