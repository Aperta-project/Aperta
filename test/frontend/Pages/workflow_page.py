...
class WorkflowPage(AuthenticatedPage):
    ...
    def __init__(self, driver):
        ...
        self._manuscript_icon = (
            By.XPATH, ".//div[@class='control-bar-inner-wrapper']"
                      "/ul[2]/li[4]/div/div/*[local-name() = 'svg']/*[local-name() = 'path']")
        self._manuscript_link = (
            By.XPATH, "//div[@class='control-bar-inner-wrapper']/ul[2]/li[4]/a")
        ...
        self._cancel_button_overlay = (
            By.XPATH, ".//div[@class='overlay-action-buttons']/button[2]")
        ...
        self._remove_confirmation_title = (
            By.XPATH, ".//div[contains(@class, 'delete-card-title')]/h1")
        self._remove_confirmation_subtitle = (By.XPATH,
                                              ".//div[contains(@class, 'delete-card-title')]/h2")
        self._remove_yes_button = (
            By.XPATH, ".//div[contains(@class, 'delete-card-action-buttons')]/div/button")
        self._remove_cancel_button = (
            By.XPATH, ".//div[contains(@class, 'delete-card-action-buttons')]/div[2]/button")
        ...
        self._competing_ints_card = (
            By.XPATH, "//a[.//span[contains(text(),'Competing Interests')]]")
        ...
        self._preprint_posting_card = (
            By.XPATH, "//a[.//span[contains(text(),'Preprint Posting')]]")
        ...
        self._report_guide_card = (
            By.XPATH, "//a[.//span[contains(text(),'Reporting Guidelines')]]")
        ...
        self._fm_reviewer_report_card = (
            By.CSS_SELECTOR, 'div.front-matter-reviewer-report-task > a')
    ...
    def modify_column_header(self, title, blank=True):
        ...
        :param blank: if blank, replace the heading with title, if not blank, append title
        to heading
        :return: void function
        """
        column_header = self._get(self._column_header)
    ...
    def check_new_tasks_overlay(self):
        ...
        expected_author_cards = ['Additional Information', 'Authors', 'Billing',
                                 'Competing Interests', 'Cover Letter',
                                 'Data Availability', 'Early Version', 'Ethics Statement',
                                 'Figures',
                                 'Financial Disclosure', 'New Taxon', 'Preprint Posting',
                                 'Reporting Guidelines',
                                 'Reviewer Candidates', 'Supporting Info', 'Upload Manuscript']
        expected_staff_cards = ['Ad-hoc for Authors', 'Ad-hoc for Editors', 'Ad-hoc for Reviewers',
                                'Ad-hoc for Staff Only',
                                'Assign Team', 'Editor Discussion', 'Final Tech Check',
                                'Initial Decision',
                                'Initial Tech Check', 'Invite Academic Editor', 'Invite Reviewers',
                                'Production Metadata',
                                'Register Decision', 'Related Articles', 'Revision Tech Check',
                                'Send to Apex',
        ...
        for expected_card_name in expected_author_cards:
            assert expected_card_name in author_card_namelist, 'Expected card: {0}, not found ' \
                                                               'in {1}'.format(expected_card_name,
                                                                               author_card_namelist)
        for expected_card_name in expected_staff_cards:
            assert expected_card_name in staff_card_namelist, \
                'Expected card: {0}, not found in {1}'.format(expected_card_name,
                                                              staff_card_namelist)
        assert 'Changes For Author' not in author_card_namelist, \
            '"Changes For Author" found in {0}.'.format(author_card_namelist)
        assert 'Response to Reviewers' not in author_card_namelist, \
            '"Response to Reviewers" found in {0}.'.format(author_card_namelist)
        assert 'Reviewer Report' not in author_card_namelist, \
            '"Reviewer Report" found in {0}.'.format(author_card_namelist)
        assert 'SUBCLASSME' not in author_card_namelist, \
            '"SUBCLASSME" found in {0}.'.format(author_card_namelist)
