namespace :role_flows do
  ...
  task :generate_defaults => :environment do
    ...
    RoleFlow.where(title: 'My papers', query: [:admin], default: true).first_or_create
    RoleFlow.where(title: 'My tasks', query: [:incomplete, :assigned], default: true).first_or_create
    RoleFlow.where(title: 'Done', query: [:completed, :assigned], default: true).first_or_create
  end
end
