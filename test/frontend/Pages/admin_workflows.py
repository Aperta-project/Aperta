...
class AdminWorkflowsPage(BaseAdminPage):
    """
    Model the Admin page, Workflow Tab elements and their functions; and the MMT (workflow)
      definition overlay elements/styles/functions
    Provides:
      page_ready()
    ...
    def page_ready_post_journal_selection(self):
        """
        We are exceedingly slow updating the page on journal selection. Need a new method
        to indicate the page is updated
    ...
    def validate_workflow_pane(self, selected_jrnl):
        ...
        :param selected_jrnl: The name of the selected journal for which to validate
        the workflow pane
        :return: void function
        """
        ...
        # Ostorozhna: The All My Journals selection is a special case. There is no add
        # workflow button and there will be no defined jid
        ...
        if mmts:
            ...
            for mmt in mmts:
                ...
                assert mmt_id, 'No workflow named "{0}" in journal: {1}'.format(name.text,
                                                                                journal.text)
                # TODO: validate Last Update label and date stamp
                # PSQL: last_update = select latest updated_at from phase_templates where
                # manuscript_manager_template_id=mmt_id (This doesn't seem correct - ask a dev)
                ...
                page_mmt_active_paper_count = \
                    int(mmt.find_element(
                        *self._admin_workflow_mmt_number_of_ms_and_label).text.split(' Active')[0])
                ...
                # The hover color of the mmt thumbnails is not present in the approved palette
                #   in the style guide. Currently only a question in APERTA-8989.
    ...
    def _validate_mmt_definition_overlay_items(self):
        ...
        for column in columns:
            ...
            time.sleep(1)
            # For a reason I can't fathom, the first click is not always registered,
            # second is always.
            col_title.click()
            col_title.click()
    ...
    def add_new_mmt_template(self, commit=False, mmt_name='',
        ...
        :param preprint_eligible: bool, Whether the mmt supports preprint functions, including
        export
        ...
        if not commit:
            ...
            # If this mmt template already exists, this save should return an error and the name
            # link won't exist
            save_template_button.click()
            # time to execute the save, then given the above comment, set timeout temporarily low
            #  since the error pops immediately in in that data condition
            time.sleep(1)
            self.set_timeout(2)
            try:
                logging.info(
                    'The following message will only be found if there is a particular data '
            ...
            logging.info(
                    'Adding {0} MMT with user tasks: {1}, staff tasks {2}, and that uses the '
                    'research reviewer report: {3}, with a preprint eligible setting: {4}, '
                    'automation setting for the similar check card: {5}'.format(mmt_name,
                                                                                user_tasks,
                                                                                staff_tasks,
                                                                                uses_resrev_report,
                                                                                preprint_eligible,
            ...
            if user_tasks:
                ...
                self._wait_for_text_be_present_in_element(self._card_columns,
                ...
                self._wait_on_lambda(
                        lambda: len(self._driver.find_elements(*self._check_divs)
    ...
    def delete_new_mmt_template(self):
        ...
        if mmts:
            ...
            for mmt in mmts:
                ...
                if len(mmts) > 1:
                    self._journal_admin_manu_mgr_thumb_delete = \
                        (By.CSS_SELECTOR, 'span.fa.fa-trash.animation-scale-in')
                    if name.text == 'Research<-False':
                        ...
                        confirm_delete = self._get(
                            self._journal_admin_manu_mgr_thumb_delete_confirm)
                        confirm_delete.click()
                        # If this mmt is found before the end of the list of mmt,
                        # the DOM will be stale so
                        break
                    else:
    ...
    def open_mmt(self, mmt_name):
        ...
        :param mmt_name: name of the mmt to open
        :return: void function
        """
        self._wait_for_element(self._get(self._admin_workflow_pane_title))
        mmts = self._gets(self._admin_workflow_mmt_thumbnail)
    ...
    def click_on_card_settings(self, card_locator):
        ...
        :param card_locator: card locator to find the gear icon
        :return: void function
        """
    ...
    def set_settings(self, setting):
        ...
        :param setting: dictionary with card name, setting name and setting value
        :return: void function
        """
