...
describe StandardTasks::PaperEditorTaskPresenter do
  ...
  describe "#data_attributes" do
    ...
    let(:task) do
      ...
      paper = Paper.create! title: "Foo bar",
        ...
        user: author
      task = StandardTasks::PaperEditorTask.create! completed: true,
    ...
    subject(:data_attributes) { StandardTasks::PaperEditorTaskPresenter.new(task).data_attributes }

    it_behaves_like "all tasks, which have common attributes" do
      let(:card_name) { 'standards-paper-editor' }
    ...
    it "returns custom data for paper editor task" do
      expect(data_attributes).to include({
        'editorId' => editor.id,
        'editors'  => [user_select_hash(editor)]
      })
    end
  end
end
