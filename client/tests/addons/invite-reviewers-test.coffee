...
module "Integration: inviting reviewers",
  ...
  setup: ->
    ...
    task = FactoryGuy.make("paper-reviewer-task", { phase: phase })
    paper = FactoryGuy.make('paper', { phases: [phase], tasks: [task] })

test "displays all pending invitees", ->
  $.mockjax
    url: /filtered_users/
    status: 200
    contentType: "application/json"
  ...
  click("#manuscript-manager .card-content:contains('Assign Reviewers')")
  pickFromSelect2(".overlay-main-work", "aaron@neo.com")
  click(".invite-reviewer-button")

  andThen ->
    ok find(".overlay-main-work:contains('aaron@neo.com has been invited to be a reviewer on this manuscript.')")
